...
class InventoryGenerator:
    ...
    def __init__(
        ...
        self.excluded_ocr_paths: Set[str] = set()
    ...
    def load_metadata(self) -> None:
        ...
        # Load excluded zero-length OCR list (optional)
        try:
            zero_csv = Path('.archive/targets/gpt_ocr_excluded_zero_length.csv')
            if zero_csv.exists():
                for line in zero_csv.read_text(encoding='utf-8').splitlines()[1:]:
                    if not line.strip():
                        continue
                    parts = line.split(',')
                    if len(parts) < 2:
                        continue
                    original_rel = parts[1]
                    norm = normalise_original_path(self.base_dir, original_rel)
                    if norm:
                        self.excluded_ocr_paths.add(norm)
        except Exception:
            # Best effort only; ignore parsing issues
            pass
    ...
    def build_json_originals(self, all_files: List[Path]) -> None:
        ...
        Fixed version: Each JSON file maps to exactly one original file path.
        No duplicate counting or path variations.
        """
        # Helper to index JSON files from a directory
        def _index_dir(json_dir: Path) -> None:
            for jf in json_dir.glob('DOC_*.json'):
                try:
                    obj = json.loads(jf.read_text(encoding='utf-8'))
                    original_path = obj.get('original_path', '')
                    file_format = obj.get('file_format', '')

                    if original_path and file_format:
                        ext = file_format.strip('.').lower()
                        # Avoid double extension if original_path already includes it
                        if original_path.lower().endswith(f".{ext}"):
                            full_path = original_path
                        else:
                            full_path = f"{original_path}.{ext}"
                        self.json_originals.add(full_path)
                except Exception as e:
                    print(f"Warning: Could not process {jf.name}: {e}")
                    continue

        # Process JSON files - main dir
        _index_dir(self.raw_json_dir)

        # Also include orphans directory if present (rehydration pending)
        orphan_dir = Path('02_converted/02_raw_json_orphans')
        if orphan_dir.exists():
            _index_dir(orphan_dir)
    ...
    def generate_inventory(self) -> List[Dict[str, str]]:
        ...
        for file_path in all_files:
            ...
            # Check if JSON exists (handle both bare and prefixed paths)
            json_exists = rel_root in self.json_originals or (
                f"2_機器_環境商材/{rel_root}" in self.json_originals
            )
            ...
            # Compute OCR status flag for requires_ocr items
            ocr_status = ''
            if category == 'requires_ocr':
                if json_exists:
                    ocr_status = 'done'
                elif rel_root in self.excluded_ocr_paths:
                    ocr_status = 'excluded'
                else:
                    ocr_status = 'pending'
    ...
    def save_csv(self, rows: List[Dict[str, str]], output_path: Path) -> None:
        ...
        fieldnames = [
            ...
            'mtime_iso', 'ext', 'category', 'raw_text', 'raw_json', 'ocr_status'
        ]
